r"""The setup script for the ``empix`` library.
"""
...
# For getting version of Python, for extracting command line arguments, for
# determining the OS, and for exiting script abruptly if necessary.
import sys
...
def get_git_revision():
    """Get revision hash of ``empix`` from git.
...
def read_extra_requirements():
    ...
    """
    extra_requirements = \
        {"pyopencl": gen_pyopencl_requirements(),
...
def gen_minimal_requirements():
    r"""Generate the minimal list of packages required to install ``empix``.
...
def gen_pyopencl_requirements():
    r"""Generate the list of required packages to run PyOpenCl.
    ...
    Returns
    -------
    pyopencl_requirements : `array_like` (`str`, ndim=1)
        The required packages to run PyOpenCl.
    """
    pkg_names = (("pyopencl[pocl]",)
                 if sys.platform.startswith("linux")
                 else ("pyopencl",))

    pyopencl_requirements = []
    for pkg_name in pkg_names:
        try:
            get_distribution(pkg_name)
        except DistributionNotFound:
            pyopencl_requirements.append(pkg_name)
    
    return pyopencl_requirements
...
def clean():
    if sys.platform.startswith("win"):
        ...
        os.system("rmdir /s /q ./empix.egg-info")
        os.system("del /q ./*.pyc")
        os.system("del /q ./*.tgz")
        os.system("del /q ./empix/_version.py")
    else:
        os.system("rm -vrf ./build ./dist ./*.pyc "
                  "./*.tgz ./*.egg-info ./empix/_version.py")
...
def setup_package():
    ...
    description = \
        ("Contains tools for analyzing electron microscopy data that are not "
