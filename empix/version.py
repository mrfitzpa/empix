...
def _get_version_summary():
    ...
    -------
    summary : `str`
        Version summary of ``empix``.
    """
    # Check versions of empix.
    from . import _version
    if _version.version != version:
        raise ValueError("empix version has changed since "
                         "installation/compilation")



    # Generate summary.
    summary = ("empix {empix_ver!s};\n"
               "git revision {git_rev!s} using\n"
               "python {python_ver!s}")
    
    summary = summary.format(empix_ver=full_version,
